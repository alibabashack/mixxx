...
class EncoderSettings {
    ...
    [[nodiscard]] virtual QList<int> getQualityValues() const { return QList<int>(); }
    [[nodiscard]] virtual int getQuality() const { return 0; }
    [[nodiscard]] virtual int getQualityIndex() const { return 0; }
    // Returns the list of compression values supported, to assign them to the slider
    [[nodiscard]] virtual QList<int> getCompressionValues() const { return QList<int>(); }
    // Sets the compression level
    virtual void setCompression(int compression) {  Q_UNUSED(compression); }
    [[nodiscard]] virtual int getCompression() const { return 0; }
    // Returns the list of radio options to show to the user
    [[nodiscard]] virtual QList<OptionsGroup> getOptionGroups() const { return QList<OptionsGroup>(); }
    // Selects the option by its index. If it is a single-element option, 
    // index 0 means disabled and 1 enabled.
    // Return the selected option of the group. If it is a single-element option, 
    // 0 means disabled and 1 enabled.
    [[nodiscard]] virtual int getSelectedOption(const QString& groupCode) const {
        Q_UNUSED(groupCode);
        return 0;
    }

    [[nodiscard]] virtual ChannelMode getChannelMode() const { return ChannelMode::AUTOMATIC; }
